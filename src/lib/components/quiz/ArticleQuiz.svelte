<script lang="ts">
	...
	const placeholder = $derived(currentMode.placeholder);
	const instruction = $derived(currentMode.quizDescription);
	...
	const currentPattern = $derived<ArticlePattern | null>(
		current ? getArticlePattern(current.noun) : null
	);
	const promptNumber = $derived(getPromptNumber(articleMode));
	const promptLabel = $derived(
		current ? (promptNumber === 'plural' ? current.noun.plural : current.noun.singular) : ''
	);
	const promptGender = $derived(
		current ? (current.noun.gender === 'm' ? 'Masculine' : 'Feminine') : ''
	);
	const promptPatternLabel = $derived(currentPattern ? getArticlePatternLabel(currentPattern) : '');
	...
	const captureAnswerInput = (node: HTMLInputElement) => {
		answerInput = node;
		return {
			destroy() {
				if (answerInput === node) {
	...
	const submit = () => {
		...
		const expected = buildArticlePhrase(articleMode, current.noun);
...
<section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
	<div class="flex flex-wrap items-center justify-between gap-4">
		<div>
			<h3 class="text-xl font-semibold text-slate-900">Article Challenge</h3>
	...
	<div class="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
			...
			<div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
				...
				{#if current}
					<div class="mt-2 flex flex-wrap items-center gap-2">
						<p class="text-lg font-semibold text-slate-900">{promptLabel}</p>
						<span
							class="inline-flex items-center rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[0.65rem] font-semibold tracking-[0.2em] text-slate-700 uppercase"
						>
							{promptGender}
						</span>
						<span
						...
						<span
							class={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[0.65rem] font-semibold tracking-[0.2em] uppercase ${
								currentPattern === 'vowel'
									? 'border-sky-200 bg-sky-100 text-sky-700'
									: currentPattern === 'special'
					...
					<p class="text-sm text-slate-500">Meaning: {current.noun.english}</p>
				{:else}
					<p class="mt-2 text-sm text-slate-500">No nouns loaded.</p>
				{/if}
			</div>
		...
		<div class="space-y-4">
			...
			<div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
				<p class="text-xs tracking-wide text-slate-400 uppercase">Tips</p>
				<ul class="mt-3 space-y-2 text-sm text-slate-600">
					<li>Type the full phrase, not just the article.</li>
					<li>Watch masculine special starts: z, s + consonant, gn, ps, x, and y.</li>
					<li>Vowel-start nouns often collapse with an apostrophe: l' or un'.</li>
					<li>Each mode drills one target: definite singular, definite plural, or indefinite.</li>
				</ul>
			</div>
		</div>
	</div>
